...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, image_size=IMG, patch_size=PATCH, width=384, layers=9, heads=6,
        ...
        # FIX: Используем nn.Linear вместо сырого nn.Parameter для стабильного проецирования/классификации.
        # Для сохранения CLIP-совместимости по именам аттрибутов, можно обращаться к self.proj.weight
        self.proj_layer = nn.Linear(width, output_dim, bias=False)
        
        # Инициализация весов проектора по принципу CLIP / Xavier
        nn.init.normal_(self.proj_layer.weight, std=scale)
        
    @property
    def proj(self):
        """Геттер для сохранения полной совместимости интерфейса CLIP (model.proj)."""
        return self.proj_layer.weight.t()
    ...
    def features(self, x):
        ...
        for blk in self.blocks:
            x = blk(x)
        return self.ln_post(x[:, 0])                           # take the CLS token

    def forward(self, x):
        return self.proj_layer(self.features(x))               # [N, output_dim]
